...
namespace MyFrameWork.Common
{
    public class SecureHelper
    {
        ...
        /// <returns></returns>
        public static string ToSha1OrderByAscii(object source)
        {
            var ps = source.GetType().GetProperties().OrderBy(a => a.Name);
            var result = "";
            foreach (var x in ps)
            {
                if (result == "")
                    result = string.Format("{0}={1}", x.Name, x.GetValue(source, null));
                else
                    result = string.Format("{0}&{1}={2}", result, x.Name, x.GetValue(source, null));
            }
            return FormsAuthentication.HashPasswordForStoringInConfigFile(result, "SHA1"); 
        }
    }
}
